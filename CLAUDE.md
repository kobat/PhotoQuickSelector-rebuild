...
- 構成:
  ...
  - `tests/PhotoQuickSelector.Core.Tests/` … xUnit（137 件）
...
`dotnet test` **137 件緑**（Core＋リンク参照の `PreviewViewport`/`DecodeGate`）。
...
- ステータスバー右端の**ハンバーガーメニュー**（`&#xE700;`）から上記トグル/外部連携/設定をクリック実行も可（ショートカット併記）。
  ...
  EXIF 詳細パネル（トグル）と構図グリッド（種類4択/基準2択ともラジオ選択。旧・巡回式2項目は廃止）を集約。
...
- プレビュー中（マウス）: メイン大画面の**シングルクリック**＝フィット⇄ズーム切替（`Z` と同一倍率）/ **ダブルクリック**＝100%
  ...
  **メイン画像の右クリック**＝プレビュー専用メニュー（`PreviewControl.ContextMenu`。**対象は常に焦点の1枚**＝選択集合は
  ...
  完全全画面・EXIF詳細パネル・情報オーバーレイ・構図グリッド・フラグ/レーティング/カラーラベル・ファイル系。
