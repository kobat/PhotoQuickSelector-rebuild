...
- 構成:
  ...
  - `tests/PhotoQuickSelector.Core.Tests/` … xUnit（312 件）
...
**Phase 1〜4 すべて完了**。旧アプリの機能同等＋既知バグ改善を達成し、**v0.2.3** として公開向け発行済み
（v0.1.0＝2026-07-02 初回公開／v0.2.0＝2026-07-12／v0.2.1＝2026-07-30／v0.2.2＝2026-08-22＝メモリ削減の一連の対応
〔バッファプール・WIC 直接デコード・掃除係・ハードリミット〕／**v0.2.3＝2026-09-17＝鮮鋭度スコア**）。
`dotnet test` **312 件緑**（Core＋リンク参照の `PreviewViewport`/`DecodeGate`/`PixelBufferPool`/`MemoryJanitor`/
...
- **パフォーマンス**: サムネイル＝圧縮バイト常駐＋可視分デコード（容量固定 LRU）／プレビュー先読み
  ...
  **メモリベンチマークモード（membench）**（2026-08-07。Debug 限定。設定フォルダの `membench.json`
  トリガーで起動時に自動でフォルダ読み込み→ `MemoryLog` 記録→写真送り連打/アイドル待機のステップ
  実行→終了する。`tools/membench/Run-MemBench.ps1` が条件（キャッシュ予算/プール比率/緊急GC閾値/
  ハードリミット）をスイープし `Analyze-MemBench.ps1` が結果を集計。同日の初回スイープ 20 ラン
  実測＝**支配的因子はプール比率で膝は 40〜50%**〔現既定 20%。50% でブロッキング GC ほぼ消滅・
  DISCARD 1/4。旧「プール拡大は効かない」推定は 20% 止まりの検証だった〕。翌 2026-08-08 に
  ナビ間隔を描画スキップしない 500ms へ是正（`-NavIntervalMs`。120ms は settle 150ms 未満で
  先読み不発だった）し、サイズばらつき 3 水準（全同一/2サイズ混在/全異寸法＝合成データ生成
  `New-MemBenchTestData.ps1`）×プール比 0/20/50% でも検証＝**プール 0% は全水準で最悪
  〔ブロッキング GC 計 10〜14 秒〕・40〜50% は全水準で最適〜無害**。同 2026-08-08 に別 PC
  （KOBAPOCKET4・RAM 23.6GB）でも全 11 ラン再実行し**結論を再現**＝プール 0% 最悪・40〜50% 最適。
  新知見＝数千枚フォルダは常駐上乗せで GC コミットが HHL 3.5GiB の 98〜99% に張り付き OOM 落ちし得る
  （4000 枚の岩国で 2 ラン確率的にクラッシュ・再試行成功）。既定値はユーザー判断で
  **20% 据え置きに確定**（2026-08-22。実使用では 20% でほぼ十分で、同程度のキャッシュを保持するには
  総予算を増やすことになりメモリ使用量増のデメリットの方が大きい、との実機所感）。
  詳細は HISTORY.md「メモリベンチマークモード（membench）」節）
...
**鮮鋭度スコア（ピント・ブレの数値化）＝v0.2.3 で公開（2026-09-12〜17）**。Core `SharpnessAnalyzer`（閾値付き Tenengrad・
全体/AF 窓/最大タイル＋異方性・タイル行並列・決定的・中断可）と採点コンソール `tools/sharpness/SharpnessBench`
（WIC 直デコード＋各ステップ ms を TSV 出力。比較用に `SharpnessMetrics`＝ラプラシアン分散/Brenner/再ぼかし法/エッジ幅も
同時採点）＝フェーズ①②。**フェーズ④＝本体組み込み（2026-09-13）まで完了**: `S` キー／両メニューで
なし→Tenengrad のみ→全手法 を巡回し、ルーペ上オーバーレイ・画像情報パネル「鮮鋭度」グループ・詳細情報オーバーレイに表示。
...
実機目視はユーザーが 2026-09-17 に確認済み（問題なし）。フェーズ③（目視突き合わせ）は本体表示で行う。
連写グループ内相対値・グリッドのバッジ・DB v3 は未着手。詳細と申し送りは HISTORY.md「鮮鋭度スコア」各節。
**改善①（2026-09-13）＝被写体領域の方向別エッジ幅**: 一方向ブレの写真で最大タイルが「動きに平行な生き残りエッジ」を拾い
順位を誤る反例（DSC04510 vs 04515）への対策。Core `SubjectRegionAnalyzer`（エッジ密度上位タイル＝被写体領域・勾配方向 8 区分の
エッジ幅中央値の最悪値＝推定ブレ幅 px・方向比・AF 窓エッジ数）＋採点コンソール列＋xUnit 11 件（**289 件緑**）まで完了。
151 枚で検証し目視と整合（Artifact「被写体領域ブレ幅 目視突き合わせ」・ユーザー確認済み）。**本体組み込みも完了**（2026-09-13。最大タイル併記・
ルーペ＋ナビにシアンの被写体領域枠・AF 窓「エッジなし」注記・settle 後の焦点写真のみ計算。実機目視済み・テスト 290 件緑）。詳細は HISTORY.md「鮮鋭度スコアの改善①」2 節。
**縮小画像（フィット表示相当）での Tenengrad は検証のうえ保留**（2026-09-17。効果薄＝ユーザー判断。Core `BgraDownscaler` と
採点コンソールの `fit_*` 列は残置・本体未参照。HISTORY.md「縮小画像（フィット表示相当）での Tenengrad の検証」節）。
