...
**次の候補（未着手）**:
- **ネイティブメモリ肥大の再調査**（2026-08-02 の調査で「`InMemoryRandomAccessStream` のバッファが
  GC 待ちで滞留」〔ファイルサイズ約2倍/デコード〕は実証済み・`BitmapDecoder` 犯人説は棄却済み。
  しかし対策 G〔`MemoryStream.AsRandomAccessStream()` 置換〕でも実機で「ネイティブ行が高くなる
  ことがある」＝**別要因あり**のため、G はいったん差し戻し済み（コードは従来どおり）。
  現在**ユーザーが再現方法を確認中**。再開時は **HISTORY.md「実験①」「恒久対応＝マネージド
  ストリーム化（差し戻し済み）」節を必ず読むこと**（測定誤差の可能性・プローブ対象外だった経路・
  再現時の記録観点を申し送り済み）。scratchpad のプローブ一式はセッション限りのため必要なら再構築）。
