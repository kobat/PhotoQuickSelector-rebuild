...
`dotnet test` **190 件緑**（Core＋リンク参照の `PreviewViewport`/`DecodeGate`/`PixelBufferPool`）。
...
- **案 K（WIC 直接デコード）の実機確認⑤**（2026-08-03。再現手順〔岩国_100MSDCF で右キー連打〕で
  ネイティブ行が連打中も固定費水準のまま積み上がらないこと・停止後も掃除なしで低いままなこと・
  カクつきがないこと・画の見た目が従来と同一なことを確認する。案 J の GC 掃除は撤去済みなので
  gen2 回数は増えないのが正常）。
...
**次の候補（未着手）**:
- **ネイティブメモリ肥大＝案 K（WIC の COM 直接呼び出し化）実装済み・実機確認⑤待ち**（2026-08-03。
  原因＝「デコード 1 回ごとの WinRT ストリーム＋デコーダのネイティブ滞留 ~20MB/枚 がファイナライザ
  待ちで積み上がる」で、GC 起動系（案 J: gen1／gen2 BGC・周期 16／8）も H2（ストリーム使い回し）も
  実機で連打中の最悪 ~2GB を抑えられず全て差し戻し/撤去。最終対策＝新規 `Controls/WicPixelDecoder.cs`
  で WIC を COM interop 直接呼び出しし全ネイティブを確定解放（GC 非依存・滞留ゼロ）。
  検証＝WinRT 経路と 32 件 byte 単位完全一致（縦位置回転込み）・確定解放実測済み・約 2 倍速
  （41ms/枚）。**落とし穴: `CopyPixels` を `[Out] byte[]` で宣言するとプロセスごと落ちる**＝
  ピン止め＋IntPtr 渡し必須。未検証＝非 sRGB の色変換パス・ミラー系 orientation（安全側フォールバック
  あり）。着手/修正時は **HISTORY.md「再現手順の確立と原因特定」〜「対策③＝案 K」節を必ず読むこと**。
  **教訓: GC 挙動に依存する対策の検証はプローブでは代表できない**〔2 回乖離した。案 K のような
  決定的処理のパリティ/解放検証はプローブで代表できる〕）。
