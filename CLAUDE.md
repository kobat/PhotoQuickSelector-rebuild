...
- **App（主要機能）**: 左右分割UI（フォルダツリー／お気に入り／最近）／サムネイルグリッド／プレビュー
  ...
  EXIF 詳細パネル（`E`・右上ルーペと排他・全生タグ表示・焦点ごとに遅延解析＋常駐）
...
**実機確認が未了の項目**:
- 先読みキャッシュの解凍爆弾ガード（2026-07-07。巨大宣言寸法のテスト画像での再現確認は未実施＝作れば `C`
  オーバーレイで観察可能。通常画像への無影響はビルド＋テスト緑で確認済み）。
- EXIF 詳細パネル（`E`）の目視（2026-07-08）。ビルド緑＋Core テスト 105 件緑＋`dotnet run` で起動継続（＝XAML
  パース/インスタンス化は成功）まで確認。ただしこのセッションでは packaged の `winapp run` が環境要因で
  起動時クラッシュ（stash したベースラインでも同症状＝当変更起因ではない）、unpackaged `dotnet run` は
  起動するが computer-use スクショが AUMID 不一致でマスクされ**パネルの見た目・E トグルの実挙動は未目視**。
  次回、ユーザーの通常フロー（BuildAndRun）で E を押して確認すること。
...
- プレビュー中: `E` EXIF 詳細パネル（右上ズームルーペと排他表示。全生タグを ListView で列挙）トグル。既定は非表示
  （ルーペ優先）。データは焦点写真ごとに遅延解析＋常駐（往復で即表示・切替のフィーリングを損ねない）。`AppSettings.ShowExifPanel` に永続化
