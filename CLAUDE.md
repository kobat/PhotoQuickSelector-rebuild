...
- **メモリ掃除係の実機確認**（2026-08-04 実装。確認ポイント＝①サイズ混在地点の連打で WS が
  「予算＋閾値 512MB」水準で頭打ちになる ②連打中のカクつきが増えない ③操作をやめて ~25 秒で
  オーバーレイに「アイドル GC」が出て基準値へ戻る ④グリッドスクロール中に完全 GC が当たらない。
  判定は補正済みオーバーレイの GC 直後基準で。詳細は HISTORY.md「メモリ掃除係」節）。
...
**次の候補（未着手）**:
- **バッファプールのサイズクラス化＝プールミス削減**（2026-08-05 のメモリログ解析で、連打中の
  WS ピーク（~4.6GB）の主因は**プールミス時の 190MB/80MB 級新規確保のバースト**（総ゴミの 85%）と
  確定＝背景 gen2 が回収で速度負けする。実測上フレームサイズは 2 種だけなので、完全一致プールを
  サイズごとに別枠で保持する小改修でミスの大半を潰せる見込み。読み捨てファイル読み込みは 12% で
  主因ではなく棚上げ。詳細は HISTORY.md「再記録ログでの検証結果」節）。
