...
HISTORY.md「実機確認⑤と『ネイティブ』積み上がりの正体」節。）
...
**次の候補（未着手）**:
- **連打時の WS 肥大の残り＝プールミスの LOH 変動への対策**（2026-08-04 に切り分け確定。
  案 K〔WIC 直接デコード〕は WinRT デコーダの滞留を正しく消しており実機確認⑤合格。残るのは
  プールがバイト長**完全一致**でしか再利用しないため、画像サイズ混在地点（190MB と 84MB）でミスが
  連発し、新規 LOH 確保＋GC の decommit 遅延で WS が予算の 2 倍近くまで膨らむ現象。M オーバーレイの
  「ネイティブ」行は直前 GC 時点のスナップショットを引く算式のためこれをネイティブに誤表示する
  〔GC 直後の値どうしでしか比較できない〕。実測＝プール割合 0% でどこでも再現／75% で WS 3.3〜3.5GB
  頭打ちに改善（ただしキャッシュ予算が保持窓を割る副作用）。対策候補＝プール割合/予算の既定見直し・
  サイズ別在庫上限・オーバーレイ算式/注記の改善。詳細は HISTORY.md「実機確認⑤と『ネイティブ』
  積み上がりの正体」節。案 K 自体の未検証項目（非 sRGB 色変換・ミラー系 orientation）と
  `CopyPixels` の落とし穴（`[Out] byte[]` 宣言でプロセスごと落ちる＝ピン止め＋IntPtr 必須）は
  据え置きで HISTORY.md「対策③＝案 K」節参照）。
