...
`dotnet test` **200 件緑**（Core＋リンク参照の `PreviewViewport`/`DecodeGate`/`PixelBufferPool`/`MemoryJanitor`）。
...
- **パフォーマンス**: サムネイル＝圧縮バイト常駐＋可視分デコード（容量固定 LRU）／プレビュー先読み
  ...
  **メモリ掃除係**（`Controls/MemoryJanitor`＝2026-08-04・3 段構成化 2026-08-05〜06。①プールミスで
  捨てた累積 512MB で背景 gen2 GC ②実測ゴミ概算〔`GetTotalMemory` − キャッシュ・プール・デコード中
  貸出＝`PreviewBitmapCache.ResidentBytes`〕が閾値〔`AppSettings.BlockingGcThresholdMB`＝既定 512・
  0 で無効・設定＞高度な設定〕を超えたら＝背景 GC の速度負け時のみブロッキング gen2〔メモリログに
  kind=`blk`・再武装ガード＝捨てバイト閾値半分・Tick 5 秒周期でも判定〕③最終操作から 20 秒の
  アイドルで `Ctrl+M` 相当の完全 GC〔結果はオーバーレイに「アイドル GC」表示〕。連打中は
  「予算＋閾値＋decommit 遅れ」で頭打ちに・アイドルで基準値へ戻す。※当初の gen2 完了カウント判定は
  実機で不発火（発行・完了とも捨て量連動で情報を持たない）と判明し実測ゴミ量判定へ作り替え。
  プール割合の既定は据え置き〔サイズ混在では完全一致プールの拡大は構造的に効かないため〕。
  詳細は HISTORY.md「メモリ掃除係」「メモリ掃除係のブロッキング段」節）／
...
- **メモリ掃除係のブロッキング段**（2026-08-05 追加→実機で不発火→2026-08-06 に判定を実測ゴミ量へ
  作り替え→実機確認②で**マネージドの上限保証は成立**〔blk×8・停止 26〜58ms・ゴミは閾値近傍で頭打ち〕、
  ただし **WS ピークは decommit 遅れ（free だが committed のページが数秒残る）で不変**〔4.60GB〕。
  経緯は HISTORY.md「メモリ掃除係のブロッキング段」節。現在 blk の Aggressive 格上げ（decommit
  同期化）の効果とカクつきを実測確認中）。
