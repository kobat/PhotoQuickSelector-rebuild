...
- **メモリ掃除係のブロッキング段の実機確認**（経緯: 2026-08-05 追加→不発火→実測ゴミ量判定へ
  →実機確認②で上限保証成立も WS ピークは decommit 遅れで不変→実機確認③で Aggressive 格上げの
  効果確認→2 段判定（blk=Forced／agr=Aggressive のモード自動選択）を試すも実機確認④で
  **Forced 後の返却予定ページが計測に写らない盲点**が判明→**常時 Aggressive で確定**〔2026-08-06〕。
  詳細は HISTORY.md「メモリ掃除係のブロッキング段」節。残る確認ポイント＝①WS 水準が実測③
  （3.3〜3.9GB 中心・4GB 超はサブ秒）と同等か ②blk 連発後の滞留（4.3GB×2 秒）が再発しないか
  ③停止時間の分布（~140〜300ms）④静穏時の過発火が無いか）。
