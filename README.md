...
- **鮮鋭度スコア（ピント・ブレの数値化）**
  - 画像のエッジの強さ（Tenengrad）から鮮鋭度を数値化し、最も鮮鋭な領域と被写体領域を枠で示します。被写体領域からは一方向ブレの推定幅（px）も算出します。
  - プレビュー中に `S` で表示を切替（なし→コンパクト→基本→全手法）。`Alt+F` で最も鮮鋭な位置へ移動できます。連写のどれがいちばんピントが来ているかの当たりを付ける用途です。
- **キーボード中心の操作**
