...
## EXIF 詳細パネル（全生タグ表示・`E` キー）

- **経緯**: ステータスバー／`I` オーバーレイには厳選 EXIF（焦点距離・絞り・SS・ISO・カメラ/レンズ等）を既に出して
  いるが、「全生タグを見たい」という要望。案検討の結果、**右上ズームルーペ（`ZoomCanvas`）と排他でトグルする
  パネル**（配置案 A）に決定。トグルキーは `E` 単独（`Ctrl+E`/`Alt+E`/`Ctrl+Alt+E` は外部連携で使用済みだが
  修飾子なし `E` は未使用）。
- **速度設計（このアプリの肝＝切替の速さを損ねない）**: 全生タグは厳選項目と違い `ImageMetadataReader.ReadMetadata`
  の**ファイル再読み（数 ms・I/O）**が要る唯一の経路。hot path から外すため:
  1. パネル非表示時は一切解析しない（`ShowExifPanel==false` なら `ScheduleExifRefresh` が即 return）。
  2. `Task.Run` でバックグラウンド解析（UI を塞がない）。
  3. `PhotoItemViewModel` に**遅延生成＋常駐**（`_rawExif`／`EnsureRawExifAsync`。サムネイルバイト
     `_thumbnailBytes` と同じ重複排除パターン）。一度見た画像は往復で即表示。フォルダ再読込で自然に破棄＝別途 LRU 不要。
  4. 焦点変更ごとに ~120ms **デバウンス**（`_exifDebounceTimer`）＋**陳腐化ドロップ**（解析完了時に焦点が
     変わっていたら結果を捨てる）。連打スクラブ中は中間コマを解析せず、止まった画像だけ解析。
- **Core**: `MetadataReader.ReadRawDump(path)` を追加（全 Directory→全 Tag を `ExifDirectoryDump`/`ExifTag`
  レコードで列挙・空 Directory 除外・失敗時空リスト）。xUnit 2 件追加（欠損パスで空／実画像で Exif・Sony 列挙）。
- **UI**: `PreviewControl.xaml` の右上スロット（`Grid.Row=0`）に `ExifPanel`（不透明・`ExifPanelVisibility`）を
  ルーペの上へ重畳。中身は**見出し行＋タグ行を平坦化した仮想化 ListView**（`ExifRow`＋`ExifRowTemplateSelector`
  で描き分け・タグ数百でも軽い・`IsTextSelectionEnabled` で値コピー可）。`ZoomCanvas_Draw` はパネル表示中は
  early-return して無駄描画を抑止。
  - **落とし穴（起動時 XAML クラッシュ回避）**: テンプレート/セレクタを `ListView.Resources` に置き
    `ItemTemplateSelector="{StaticResource ...}"` を ListView 属性で参照すると、属性→子要素の解析順で
    **前方参照**になり得る。`UserControl.Resources`（ListView より前に評価）へ置くこと。
- **配線**: `MainViewModel.ShowExifPanel`（`ExifPanelVisibility`＋`AppSettings.ShowExifPanel` 永続化・既定 false＝
  ルーペ優先）／`PreviewControl.Input.cs` に `E`／`PhotoStatusBar` メニューに `ExifToggleItem`／resw 日英。
  `OnViewModelPropertyChanged` の `FocusedPhoto` で `ScheduleExifRefresh`、`ShowExifPanel` 変化と
  プレビュー入場で即 `RefreshExifPanelAsync`。
- **状態**: `BUILD SUCCEEDED`／`dotnet test` **105 件緑**。`dotnet run`（unpackaged）で起動継続を確認（XAML
  パース/インスタンス化成功）。**目視未了**: このセッションでは packaged `winapp run` が環境要因で起動時
  クラッシュ（stash したベースラインでも同症状＝本変更起因ではない）、unpackaged は computer-use スクショが
  AUMID 不一致でマスクされ、パネルの見た目・`E` トグルの実挙動は未確認。次回ユーザー通常フローで要目視。
