...
### プール比率の既定値は 20% 据え置きに確定（2026-08-22）

membench の実測（膝 40〜50%・0% は全水準最悪）を踏まえてユーザーが実使用で判断した結果、
**既定 20% のまま据え置き**とした。理由＝実使用では 20% でほぼ十分に感じられ、プール比を上げると
同じ総予算ではキャッシュ枠が同額減る＝同程度のキャッシュを保持するには総予算を増やすことになり、
メモリ使用量増加のデメリットの方が大きい、との実機所感。ベンチ結果（40〜50% が最適〜無害）は
記録として残し、メモリに余裕がない環境・サイズ混在フォルダで問題が出た場合の調整指針とする。

## メモリ関連の初期値の変更（緊急GC閾値・ハードリミットを既定 0 へ）（2026-08-22）

ユーザー指定により、メモリ関連の設定初期値（`AppSettings` の既定値＝新規インストール時・
`settings.json` 未記載時に効く値）を以下へ変更した。既存ユーザーの保存済み設定値は影響を受けない。

| 設定 | 変更前 | 変更後 |
|---|---|---|
| キャッシュ容量予算 `CacheBudgetGB` | 2.5GB | 2.5GB（据え置き） |
| バッファ再利用プール割合 `CachePoolRatioPercent` | 20% | 20%（据え置き） |
| 緊急メモリ回収のしきい値 `BlockingGcThresholdMB` | 512 | **0＝無効** |
| メモリ使用量の上限 `HeapHardLimitGB` | 3.5GiB | **0＝無効** |

- 変更箇所は `AppSettings.cs` の既定値 2 行のみ。掃除係のブロッキング段（②）とハードリミットが
  既定でオフになる＝背景 gen2・アイドル GC（①③）と `ConserveMemory`=7 は従来どおり働く。
  CLAUDE.md の該当節「既定 512」「既定 3.5GiB」の記述も更新した。
- `HeapHardLimitPolicy` の下限クランプ・0 無効の素通しはコード不変（既定値が 0 になっただけ）。
  `HeapHardLimitPolicyTests` の「既定値 3.5GiB」に言及していたコメントを「代表値」表現へ修正。
- ※この変更は `claude/memory-initial-values-ofmn2g` ブランチに作られたまま v0.2.2 の初回タグに
  取り込み漏れとなり、判明後に main へマージして**v0.2.2 をリリースし直した**（下記リリース節参照）。

## v0.2.2 リリース（2026-08-22）

v0.2.1（2026-07-30）以降の 22 コミットをまとめて **v0.2.2** として公開（GitHub Release・Pre-release 継続）。
csproj `<Version>` を 0.2.2 に更新（`Package.appxmanifest` は前回リリース直後に 0.2.2.0 へ上げ済み）。
発行・添付は既定どおり**単一ファイル版**（`win-x64-singlefile`・zip には exe＋LICENSE＋THIRD-PARTY の
3点のみ・`.pdb` 除外）。

- **v0.2.1 → v0.2.2 の主な変更**（利用者視点）: 中身はほぼ「メモリ削減の一連の対応」
  - **メモリ使用量の大幅削減**: 先読みキャッシュのバッファ再利用プール化（`PixelBufferPool`）＋
    GC 設定 `ConserveMemory`=7＋メモリ掃除係（`MemoryJanitor`＝3 段構成）で、写真送り連打時の
    ワーキングセット高止まりを解消。アイドル 20 秒で基準値へ自動復帰
  - **プレビューデコード高速化（約 2 倍）**: WIC の COM 直接呼び出し化（`WicPixelDecoder`。
    ネイティブメモリ滞留の恒久対策を兼ねる）
  - **マネージドヒープのハードリミット**（OOM 的肥大の保険）と**緊急 GC 閾値**を設定＞高度な設定に追加。
    **既定はいずれも 0＝無効**（上記「メモリ関連の初期値の変更」＝リリース直前にユーザー判断で
    導入時の 3.5GiB／512MB から変更）。必要な環境では設定から有効化する
  - メモリ計測系（デバッグ用）: `M` オーバーレイに「ネイティブ」行・`Ctrl+Shift+M` の時系列ログ
    （TSV）・Ctrl 系キーの誤爆防止ゲート（オーバーレイ表示中のみ有効）
- **開発向け（配布物には非影響）**: メモリベンチマークモード membench（Debug 限定・
  スイープ/分析/データ生成ハーネス `tools/membench/`）
- リリース作業: バージョン更新コミット（csproj のみ・`-dev` を外す）→ `main` へ push →
  `v0.2.2` タグ → `dotnet publish -p:PublishProfile=win-x64-singlefile` → `gh release create`
  （Pre-release・`PhotoQuickSelector-0.2.2-win-x64.zip` 添付）→ 次サイクル開始コミット
  （csproj 0.2.3-dev・appxmanifest 0.2.3.0）。`dotnet test` 229 件緑。
- **リリースし直し（同日）**: 初回タグ（`e1c35d3` 起点・既定 3.5GiB／512MB のまま）に
  `claude/memory-initial-values-ofmn2g` ブランチ（初期値を 0＝無効へ変更）の取り込み漏れが
  判明したため、同ブランチを main へマージ→バージョンを一時 0.2.2 へ戻すコミット→
...
## 鮮鋭度スコアの改善①＝被写体領域の方向別エッジ幅（2026-09-13）

ユーザー指摘の反例: `20231126_築城基地航空祭\101MSDCF\DSC04510.JPG`（目視ではピント良）と `DSC04515.JPG`
（全体的にブレ）で、5 手法すべての**最大タイル**が 04515 を上位にした。

### 原因の切り分け（採点コンソール＋方向別の追加実験）
- 04515 は**一方向ブレ**（異方性 2.36・構造テンソル λ2/λ1 0.44）。動きに平行なエッジ（機首・カウリング側面）は 4px の
  まま立ち、直交エッジは 11.5px。**最大タイルは生き残った平行エッジの塊を拾う**ので、max 系の集計は構造的に誤る。
- 勾配エネルギー系（Tenengrad／ラプラシアン分散／Brenner）は「コントラスト² × 立ち上がり × エッジ密度」でブレ幅そのものを
  測っていない。04515 は 366mm で機体が小さく（被写体 9 タイル vs 33 タイル）タイル内エッジ密度が高く、日向の黄色×青空で
  高コントラスト＝最大タイル値が上がる要因が重なった。
- 04515 の **AF 窓は青空に乗って Tenengrad 0**＝比較不能。「AF 窓 0＝完全にボケ」と読めてしまう表示上の問題。
- 04510 は最悪方向 9px だが機体が 1.6 倍大きく、**被写体幅に対するブレ量は約 1/3**＝「縮小なら問題ない」感覚の正体。

### 実装（Core `SubjectRegionAnalyzer`・`SubjectRegionOptions`・`SubjectRegionScore`）
- **被写体領域**＝タイル別エッジ画素数（mag ≥ 閾値）が最大タイルの `DensityRatio`（既定 25%）以上のタイル群。
- 被写体タイル内の全エッジ画素を勾配方向 `OrientationBins`（既定 8＝22.5° 刻み・[0,π)）に分け、勾配方向の非極大抑制
  （双線形補間で ± 単位勾配方向の mag² と比較）を通った画素で**エッジ幅**を測る＝± 勾配方向へ `WalkStep`（0.5px）ずつ
  双線形で輝度を追い、1 歩の変化が `WalkMinDelta`（0.75）未満になったら停止・往復の距離。`MaxEdgeWidth`（64）以上は破棄。
- 区分ごとの中央値のうち、`MinEdgesPerBin`（50）以上の区分の**最大＝`WorstWidth`（推定ブレ幅 px）**・最小＝`BestWidth`・
  比＝`WidthRatio`（一方向ブレ指標）。構造テンソル λ2/λ1（`AnisotropyRatio`＝任意方向の異方性。従来の x/y 比は斜め動きを
  取りこぼす）・支配的勾配方向・`Bounds`（タイル群の外接矩形）・被写体内 Tenengrad／エッジ密度／エッジ当たり mag²・
  **AF 窓のエッジ画素数**（`AfWindowEdgeCount`＝空振り検知用）を返す。
- 決定性: パス 1（タイル別エッジ数）はバンド並列で添字排他、パス 2 はタイル単位並列で結果スロット排他→タイル添字昇順で逐次
  集約・幅リストは結合後ソート。xUnit 11 件（横ブラーのみで最悪区分が 0° になる等）＝**289 件緑**。
- 全画像の Sobel を 2 回計算する（パス 1 と `SharpnessAnalyzer`）。本体組み込み時はタイル別エッジ数を Tenengrad 側の
  1 パスに相乗りさせると重複が消える（未着手）。実測 `subj_ms` 50〜67ms（50MP）。

### 採点コンソールの列追加（`tools/sharpness/SharpnessBench`・README 更新）
`subj_tiles`／`subj_x,y,w,h`／`subj_ten`／`subj_edge_density`／`subj_per_edge`／`subj_aniso`／`subj_dir_deg`／
`subj_w_worst`／`subj_w_worst_deg`／`subj_w_best`／`subj_w_ratio`／`subj_w_rel_extent`（ブレ幅÷被写体外接矩形の長辺 ‰）／
`subj_bins`（8 区分の中央値）／`af_edges`／`af_edge_density`／`rel_subj_w`／`rel_subj_extent`（小さいほど鮮鋭＝edgew 流）／`subj_ms`。
グループサマリに `best_subj=`／`best_extent=` を併記。

### 検証（DSC04489〜04639 の 151 枚・3 グループ）
- 反例ペア: 04510 ブレ幅 9.0px（方向比 2.0）／04515 11.5px（方向比 2.9・AF 窓エッジ 0）＝**目視と一致**。
- グループ 2（46 枚・366mm）: 最良 04527／04532（4.0px）、04515 は下位。等倍切り出しで 4px 組は鮮明・15.5px（04531）は明確な流れ。
- グループ 1（26 枚・600mm）: ブレ幅最良 04508（8.0）→ 04493／04495（8.5）→ 04510（9.0）。AF 窓最良は 04493＝おおむね整合。
- 目視突き合わせ用 Artifact「被写体領域ブレ幅 目視突き合わせ」（151 枚・縮小全体に被写体領域〔シアン〕／最大タイル〔オレンジ〕／
  AF 窓〔黄〕＋最大タイル中心の等倍切り出し・並び順切替）を作成＝ユーザー目視待ち。

### 申し送り
- **被写体タイルの絶対下限が無い**: 完全にピンボケでエッジがほぼ無い写真（04605〜07・04619〜20＝最大タイル 0.05）でも
  「最大の 25%」で 16〜34 タイルが被写体扱いになり外接矩形が画面全体になる（幅は `MinEdgesPerBin` で NaN になるので
  順位は壊れない）。`MinTileEdgeCount`（例: タイル面積の 0.5%）を足し、満たなければ「被写体なし」とするのが次の修正。
- 本体表示は「最大タイル（現状維持）＋被写体領域（新規行）」の併記方針で合意済み（2026-09-13）。表示案＝ルーペ／
  画像情報パネル／詳細情報オーバーレイに「被写体領域: N タイル・ブレ幅 px・方向比」、被写体外接矩形をシアン細線で
  ルーペ＋ナビへ、AF 窓はエッジ不足時に「エッジなし」表示。連写グループ内相対値の既定は「被写体ブレ幅（小さいほど良）」を推奨。
- エッジ幅の歩行は大きな面の陰影で幅が水増しされる可能性がある（600mm グループの 0° 区分が 9px 前後）。目視結果次第で
  `WalkMinDelta` の見直し、または勾配が閾値を割った時点で停止する条件の追加を検討。

## 鮮鋭度スコアの改善①＝本体組み込み（2026-09-13）

ユーザー目視（Artifact「被写体領域ブレ幅 目視突き合わせ」）で新指標の順位が感覚と合うことを確認し、
「最大タイルは現状維持＋被写体領域を併記」の合意どおり本体へ組み込んだ。**実機目視は未了**（ビルド成功・テスト **290 件緑**）。

- **Core**: `SubjectRegionOptions.MinTileEdgeFraction`（既定 0.005＝タイル面積比）を追加。最大タイルのエッジ数がこの絶対下限
  未満なら「被写体なし」（`TileCount`=0）。申し送りにあった「完全ピンボケで画面全体が被写体扱い」を解消。xUnit 1 件追加。
- **計算経路**（`PreviewControl.Sharpness.cs`）: 被写体領域は**焦点写真の確定計算（settle 後）でのみ**計算＝順序は
  Tenengrad → 被写体領域 → 比較 4 手法（全手法モードのみ）。同じリース＋CancellationToken。モード None 以外なら
  モードを問わず一度だけ計算（`PhotoItemViewModel.SubjectRegion` が null の間）。**先読み便乗フック（`OnFrameDecoded`）
  には載せない**＝1 枚 55ms を DecodeGate のスロット保持中に先読み全枚数ぶん払うのを避ける（将来グリッドバッジ等で
  全枚数が要るなら、Tenengrad のパスにタイル別エッジ数を相乗りさせてから検討）。
- **保持**: `PhotoItemViewModel.SubjectRegion`（観測可能・UI スレッドのみ）＋詳細情報オーバーレイ用 `SubjectRegionText`。
- **表示 3 箇所**: `SharpnessInfoSection` に「ブレ幅」（値 px・注記「最悪方向 N° ・ 方向比 R」）と「被写体領域」（値 N タイル・
  注記 @x,y w×h）の 2 行を**最大タイルの直後**に追加＝並びは 最大タイル→ブレ幅→被写体領域→AF 窓→全体→異方性（→比較 4 手法）。
  **AF 窓行は被写体解析のエッジ数が `AfNoEdgeThreshold`（200）未満なら注記「エッジなし」**（AF 窓が青空に乗った DSC04515 の
  ような空振りを「0＝完全にボケ」と読ませない）。詳細情報オーバーレイは `SubjectRegionText` の 1 行を Tenengrad 行の下に追加。
  ルーペ見出しは「鮮鋭度 (Tenengrad 法)」→「鮮鋭度」。
- **枠**: 被写体領域の外接矩形を**シアン**（`SubjectRegionColor`）1.5px でルーペ＋ナビに描く（`TryGetSubjectBounds`／
  `DrawSubjectRegionFrame`＝最大タイル枠と同じガード＋`TileCount`>0）。メインには描かない。ルーペの初期位置は最大タイル基準の
  まま（被写体領域では寄せない）。
- **メニュー表記**: 中間モードのラベルを「Tenengrad のみ」→「基本（Tenengrad＋被写体領域）」（ja/en）。`S` 巡回・
  `shortcuts.json` は不変。
...
## 鮮鋭度: 縮小画像（フィット表示相当）での Tenengrad の検証＝効果薄（2026-09-16〜17）

ユーザー発案「縮小画像の Tenengrad を出せば『縮小表示で見て気になるか』を数値化できるのでは」を実データで検証した。
**結論＝効果が高くなく、この方向は保留**（ユーザー判断・2026-09-17）。本体への組み込みは行っていない。

### 実装（採点コンソールのみ・未コミット時点で保留）
- Core `BgraDownscaler`（`src/PhotoQuickSelector.Core/BgraDownscaler.cs`）: 面積平均（OpenCV `INTER_AREA` 相当・任意比率・
  B/G/R 独立・A=255・四捨五入 AwayFromZero・行並列で決定的）と `FitSize`（長辺固定・拡大なし）。xUnit 22 件（全 312 件緑）。
  実装は元バッファを `ArrayPool` へ 1 回コピーする（`ReadOnlySpan` をラムダへ渡せないため）＝50MP で 15〜17ms。
  本体へ載せるなら unsafe ポインタ経由でコピーを省ける。
- `tools/sharpness/SharpnessBench`: `--fit-long`（既定 1600）/`--fit-tile`（既定 64）と列 `fit_*`（縮小画像での Tenengrad・
  被写体領域・ブレ幅）/`ratio_global`/`ratio_subj`（等倍÷縮小の Tenengrad 比）/`rel_fit_*`/`fit_downscale_ms`/`fit_ms`。README 更新。
- 151 枚のサブセットは `D:\Users\kobat\tmp_ClaudeCode用\築城_151`（101MSDCF の DSC04489〜04639 へのハードリンク）。

### 実測（築城 151 枚・1600 と 2160 の 2 水準・画面 2560×1440@150%＝通常フィット長辺≒1700／イマーシブ≒2160）
- **縮小画像で直接測った値は弁別力が弱い**。縮小側の被写体ブレ幅は等倍最鋭の 04527/04532（4px）が 8〜9px、ブレた 04515 が
  7.5px と**逆転**。縮小で面のなだらかな陰影が閾値 32 を超え「幅の広いエッジ」に化けるためで、閾値・`WalkMinDelta` の意味が
  縮尺をまたいで通用しない（下限も ~3px で飽和）。縮小側の被写体 Tenengrad も 04515 > 04510 と等倍と同じコントラスト支配。
- **等倍÷縮小の Tenengrad 比（`ratio_subj`）は等倍ブレ幅と単調**（4〜6px で 0.45 → 18〜22px で 0.17）＝縮尺不変のブレ指標に
  なり得る。ただし既にある被写体ブレ幅・再ぼかし法と同じ量を測る第三の推定量で、新情報は少ない。
- 「縮小表示で気になるか」は結局**等倍ブレ幅 × 縮尺＝表示 px** に帰着し、同一解像度の連写内では順位は等倍値と同じ。
  変わるのは閾値だけ（参考: 04527 0.74px／04510 1.67px／04515 2.13px／04531 2.87px）。
